...
from app.currency_helper import EXCHANGE_RATES, NEPAL_CITIES, NEPAL_PROPERTY_TYPES
...
def property_form_context(**kwargs): return {"cities": NEPAL_CITIES, "property_types": NEPAL_PROPERTY_TYPES, "currencies": ALLOWED_CURRENCIES, **kwargs}

@main.route("/")
def home():
    return render_template("home.html", featured_properties=Property.query.filter_by(status="available").order_by(Property.created_at.desc()).limit(6).all(), total_properties=Property.query.filter_by(status="available").count(), total_agents=User.query.filter_by(role="agent").count())
@main.route("/properties")
def properties():
    user_currency = session.get("currency", "NPR"); search_query = request.args.get("search", ""); property_type = request.args.get("type", ""); city = request.args.get("city", ""); min_price = request.args.get("min_price", type=float); max_price = request.args.get("max_price", type=float); bedrooms = request.args.get("bedrooms", type=int)
    ...
    pagination = query.order_by(Property.created_at.desc()).paginate(page=request.args.get("page", 1, type=int), per_page=12, error_out=False)
    return render_template("properties.html", properties=pagination.items, pagination=pagination, cities=NEPAL_CITIES, property_types=NEPAL_PROPERTY_TYPES, user_currency=user_currency)
...
def register():
    ...
    if request.method == "POST":
        ...
        user = User(username=username, email=email, full_name=request.form.get("full_name"), phone=request.form.get("phone"), role="user"); user.set_password(password); db.session.add(user); db.session.commit(); flash("Registration successful! Please log in.", "success"); return redirect(url_for("main.login"))
    return render_template("register.html")
@main.route("/login", methods=["GET", "POST"])
@limiter.limit("10 per hour")
def login():
    if current_user.is_authenticated: return redirect(url_for("main.agent_dashboard" if current_user.role == "agent" else "main.user_dashboard"))
    if request.method == "POST":
        user = User.query.filter_by(username=request.form.get("username")).first()
        if user and user.check_password(request.form.get("password") or ""):
            login_user(user, remember=bool(request.form.get("remember"))); flash(f"Welcome back, {user.full_name or user.username}!", "success"); return redirect(url_for("main.agent_dashboard" if user.role == "agent" else "main.user_dashboard"))
...
@login_required
def user_dashboard():
    if current_user.role != "user": return redirect(url_for("main.agent_dashboard"))
...
def user_profile():
    if request.method == "POST":
        ...
        if not email or "@" not in email: flash("Please enter a valid email address.", "danger")
            ...
            current_user.full_name = request.form.get("full_name"); current_user.phone = request.form.get("phone"); current_user.email = email
...
@login_required
def agent_dashboard():
    if current_user.role != "agent": return redirect(url_for("main.user_dashboard"))
    ...
    properties_pagination = base_properties.order_by(Property.created_at.desc()).paginate(page=request.args.get("page", 1, type=int), per_page=10, error_out=False); inquiries_pagination = inquiries_query.order_by(Inquiry.created_at.desc()).paginate(page=request.args.get("inquiry_page", 1, type=int), per_page=10, error_out=False)
    return render_template("agent_dashboard.html", properties=properties_pagination.items, inquiries=inquiries_pagination.items, properties_pagination=properties_pagination, inquiries_pagination=inquiries_pagination, stats={"total": base_properties.count(), "available": base_properties.filter_by(status="available").count(), "inquiries": inquiries_query.count(), "pending": inquiries_query.filter_by(status="pending").count()})
def populate_property(property_item):
    for field in ("title", "description", "property_type", "address", "city", "state", "zipcode", "status"):
        if field in request.form: setattr(property_item, field, request.form.get(field))
    property_item.price = request.form.get("price", type=float); property_item.currency = request.form.get("currency", "NPR"); property_item.bedrooms = request.form.get("bedrooms", type=int); property_item.bathrooms = request.form.get("bathrooms", type=int); property_item.area = request.form.get("area", type=float)
